...
from courses.views import CourseListView, CourseDetailView, search_result
...
class CourseListViewTest(TestCase):
    ...
    def test_get_queryset_api_call(self, mock_get):
        ...
        mock_get.assert_called_once_with("https://data.cityofnewyork.us/resource/fgq8-am2v.json", timeout=10)
...
class SearchResultTest(TestCase):
    ...
    def test_search_by_name(self):
        response = self.client.get(reverse('search_result'), {'q': 'Python'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_by_description(self):
        response = self.client.get(reverse('search_result'), {'q': 'data science'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['courses']), 1)
        self.assertEqual(response.context['courses'][0], self.course2)
        
    def test_search_by_keywords(self):
        response = self.client.get(reverse('search_result'), {'q': 'web'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['courses']), 2)
        self.assertIn(self.course1, response.context['courses'])
    ...
    def test_filter_by_cost(self):
        response = self.client.get(reverse('search_result'), {'min_cost': '900', 'max_cost': '1500'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['courses']), 1)
    ...
    def test_filter_by_classroom_hours(self):
        response = self.client.get(reverse('search_result'), {'min_classroom_hours': '50'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['courses']), 1)
        self.assertEqual(response.context['courses'][0], self.course2)
