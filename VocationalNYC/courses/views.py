        ...
        if not Course.objects.exists():
            ...
            try:
                ...
                for course in courses_data:
                    ...
                    classroom_hours = int(float(classroom_match.group(1))) if classroom_match else 0
                    
                    lab_match = re.search(r'Lab Hours\s+(\d+\.?\d*)', duration)
                    lab_hours = int(float(lab_match.group(1))) if lab_match else 0
                    
                    internship_match = re.search(r'Internship Hours\s+(\d+\.?\d*)', duration)
                    internship_hours = int(float(internship_match.group(1))) if internship_match else 0
                    
                    practical_match = re.search(r'Practical Hours\s+(\d+\.?\d*)', duration)
                    practical_hours = int(float(practical_match.group(1))) if practical_match else 0
...
def search_result(request):
    ...
    print("Query:", query)
    print("min_cost:", min_cost, "max_cost:", max_cost, "location:", location, "min_classroom_hours:", min_classroom_hours)
    ...
    if min_classroom_hours is not None and min_classroom_hours.isdigit():
        courses = courses.filter(courseduration__classroom_hours__gte=int(min_classroom_hours))
...
#     if min_cost is not None and min_cost.isdigit():
#         results = results.filter(cost__gte=int(min_cost))
    
#     if max_cost is not None and max_cost.isdigit():
#         results = results.filter(cost__lte=int(max_cost))
    
#     if location:
#         results = results.filter(location__icontains=location)
    
#     if min_classroom_hours is not None and min_classroom_hours.isdigit():
#         results = results.filter(courseduration_set__classroom_hours__gte=int(min_classroom_hours))
