...
class CustomUserAdmin(UserAdmin):
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).exclude(role__in=['training_provider', 'career_changer'])

@admin.register(Provider)
class ProviderAdmin(admin.ModelAdmin):
    list_display = ('provider_id','user_id','name', 'contact_firstname', 'contact_lastname', 'phone_num', 'verification_status', 'created_at')
    ...
    fieldsets = (
        ...
        ('Location & Hours', {
            'fields': ('address', 'open_time', 'website')
        }),
        ('Description', {
            'fields': ('provider_desc',)
        }),
        ('Verification', {
            'fields': ('verification_status', 'verification_file_url')
        }),
        ('Timestamps', {
